...
bool StackState::apply(Stack const& opcode) {
	...
	case Stack::Opcode::PUXC2: {
		if (std::max(std::max(1, index0), std::max(index1, index2)) < m_size &&
			m_size + 1 <= StackState::MAX_STACK_DEPTH) {
...
std::optional<int>
StackOpcodeSquasher::gasCost(int startStackSize, StackState const& _state, bool _withCompoundOpcodes) {
...
std::array<std::array<std::unordered_map<StackState, StackOpcodeSquasher::DpState>, StackState::MAX_STACK_DEPTH + 1>, 2>
	StackOpcodeSquasher::m_dp{};
...
void StackOpcodeSquasher::init() {
	for (int _withCompoundOpcodes = 0; _withCompoundOpcodes <= 1; ++_withCompoundOpcodes) {
		...
		auto addEdge = [&](auto const& opcode) { edges.emplace_back(Edge{opcode, OpcodeUtils::gasCost(*opcode)}); };
		...
		std::ranges::stable_sort(edges, [](Edge const& a, Edge const& b) { return a.gas < b.gas; });
...
std::vector<Pointer<TvmAstNode>>
StackOpcodeSquasher::recover(int startStackSize, StackState state, bool _withCompoundOpcodes) {
