...
public:
	...
	explicit StackState(int8_t _size, std::array<int8_t, MAX_STACK_DEPTH> _values):
		m_size{_size},
		m_values{_values} {
		updHash();
	}
	bool apply(Stack const& opcode);
	bool operator==(StackState const& other) const {
	...
	bool operator!=(StackState const& other) const { return !operator==(other); }
...
struct std::hash<solidity::frontend::StackState> {
	std::size_t operator()(solidity::frontend::StackState const& k) const noexcept { return k.getHash(); }
};
