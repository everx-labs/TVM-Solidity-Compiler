...
public:
	explicit StackOptimizer(bool _withDrop):
		m_withDrop(_withDrop) {}
...
private:
	...
	successfullyUpdate(int index, std::vector<Pointer<TvmAstNode>>& instructions) const;
	...
	bool m_didSome = false;
	std::vector<int> m_stackSize;
	int m_tryCatchQty = 0;
	bool m_withDrop = false;
